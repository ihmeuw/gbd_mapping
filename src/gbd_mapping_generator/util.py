...
def format_tuple_params(items, prefix='', indent=8, line_width=118):
    """Format a tuple of parameters with smart line wrapping."""
    if not items:
        return "()"
    ...
    single_line = f"({', '.join(f'{prefix}{item}' for item in items)}, )"
    if len(single_line) <= line_width - indent:
        return single_line
    ...
    for item in items:
        ...
        if current_line and current_length + item_length > line_width:
            # Start new line
            lines.append(''.join(current_line))
            current_line = []
            current_length = len(' ' * (indent + len('sequelae=(')))
    ...
    if len(lines) == 1:
        ...
        for line in lines[1:]:
            formatted_lines.append(f"{' ' * (indent + len('sequelae=('))}{line}")
        formatted_lines[-1] += ")"
